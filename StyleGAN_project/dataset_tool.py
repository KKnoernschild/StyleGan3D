...
class TFRecordExporter:
    ...
    def add_image(self, img):
        ...
        if self.shape is None:
            ...
            print('image shape ' , self.shape)
            self.resolution_log2 = int(np.log2(self.shape[1]))
            print('reslog2 shape ' , self.resolution_log2)
        ...
        for lod, tfr_writer in enumerate(self.tfr_writers):
            if lod:
                #print('in loop imsize' , img.shape)
...
def create_from_images(tfrecord_dir, image_dir, shuffle):
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames), print_progress=False) as tfr:
        ...
        for idx in tqdm(range(order.size)):
            #Read in nrrd file regardleess of resolution
            im, header = nrrd.read(image_filenames[order[idx]])
            img = np.asarray(im)
            tfr.add_image(img)
